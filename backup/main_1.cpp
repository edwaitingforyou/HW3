...
void player_fire()
{
	...
	Bullet *bullet = new Bullet(x_position_player_ship, -0.7, 0.00030, 1);
...
void enemy_fire()
{
	...
	Bullet* bullet = new Bullet(enemy_data[index]->x, enemy_data[index]->y-0.15, 0.00030, -1);
	bullets.push_back(bullet);
...
void increment()
{
	...
	for (int i = 0; i < bullets.size(); i++)
	{
		bullets[i]->y += bullets[i]->direction*elapsed*0.5;
	}
}
